...
import { GridRandomTemplate, ContrastRatio } from '@/helpers';
...
export default function Section() {
    ...
    const colorsArray: string[] = imageData.clrArr;
    ...
    // Grid template
    const template = new GridRandomTemplate(elementsToDisplay)
        .gridTemplateAreas;
    ...
    useEffect(() => {
        const firstContrast = new ContrastRatio(
            colorScheme.current.bgColor,
            colorScheme.current.accent3
        ).ratio;
        const secondContrast = new ContrastRatio(
            colorScheme.current.bgColor,
            colorScheme.current.accent1
        ).ratio;

        if (
            firstContrast < 1.9 &&
            secondContrast < 1.9 &&
            Math.max(firstContrast, secondContrast) -
                Math.min(firstContrast, secondContrast) <
                0.42
        ) {
